...
public class FastestPathAlgo {
	 ...
	 * -> Repeat
	 */
	public ArrayList<Tile> findFastestPath(int start_row, int start_col, int end_row, int end_col, RobotConstants.HEADING h, boolean explor, boolean cnt) {
		//reinit
		parentTile.clear();
		frontier.clear();
		closed.clear();
		...
		this.curTile=explored_map.getTile(start_row, start_col);
		this.curHeading = robot.getCurrentHeading();
		ArrayList<Tile> path = reversePath(goal, cnt);
//		if(!cnt){
...
//		}
	...
	public void addNeighbors(boolean explor) {
		Tile tempTile;
		int row,col;
		double g,h;
		...
		if (explor==true){
			//immediate front
			if(explored_map.checkTileSurroundingLenient(row, col) ) {
...
//		}
	 ...
	 */
	private ArrayList<Tile> reversePath(Tile goal, boolean cnt) {
        Tile temp = this.explored_map.getTile(goal.getRow(), goal.getCol());
        ArrayList <Tile> tempPath = new ArrayList<Tile>();
//        int steps = 0;
        while (true) {
            //if cnt is true, then do not set this path as ispath and not as isvisited
        	if(cnt){
        		System.out.println("Counting steps not setting path");
//				steps = steps+1;
				tempPath.add(temp);
			}
        	else{
        		System.out.println("Setting the path is true is on:");
				explored_map.getTile(temp.getRow(),temp.getCol()).setIsPath(true);
...
//				temp = parentTile.get(temp);
			...
			temp = parentTile.get(temp);
//        	if(cnt){
//        		Tile check_temp = parentTile.get(temp);
//				if(temp != null && check_temp == null){
//					temp.setCount(steps + 1);
//					tempPath.add(temp);
//
//				}
//			}
